...
func RouterConfig() *mux.Router {
	...
	router.HandleFunc("/v1/config", HandleConfigGet).Methods("GET")
	router.HandleFunc("/v1/config", HandleConfigDelete).Methods("DELETE")
	router.HandleFunc("/v1/loadconfig", HandleConfigLoad).Methods("POST")
	return router
}
...
func TestHandlePOST(t *testing.T) {
	...
	request, _ := http.NewRequest("POST", "/v1/loadconfig", bytes.NewBuffer(b))
